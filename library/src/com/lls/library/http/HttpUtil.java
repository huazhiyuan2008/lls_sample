package com.lls.library.http;

import android.content.Context;
import com.lls.library.http.depend.HttpDownloadHandle;
import com.lls.library.http.depend.HttpRequest;
import com.lls.library.http.depend.HttpRequest.ConnectionFactory;
import com.lls.library.http.depend.HttpRequest.HttpRequestException;
import com.lls.library.http.depend.HttpRequest.UploadProgress;
import com.lls.library.http.depend.HttpResponseHandle;
import com.lls.library.http.depend.HttpUploadHandle;
...
public class HttpUtil {
     ...
     * @param header   请求头
     * @param paramter 请求参数
     * @param encode   url是否encode
     * @param gzip     传输是否gzip压缩
     * @param handle   自定义回调
     * @return
     */
    public static String getMethod(String url, Map<String, String> header, Map<String, String> paramter,
     ...
     *
     * @param url     上传地址
     * @param fileKey 上传文件的key
     * @param file    上传文件
     * @return
     */
    public static String upload(String url, String fileKey, File file) {
        return upload(url, new String[]{fileKey}, new File[]{file}, null, null, null);
    }
     ...
     * @param fileKey      上传文件的key
     * @param file         上传文件
     * @param uploadHandle 自定义回调
     * @return
     */
    public static String upload(String url, String fileKey, File file, HttpUploadHandle uploadHandle) {
        return upload(url, new String[]{fileKey}, new File[]{file}, null, null, uploadHandle);
    }
    ...
    public static String upload(String url, String[] fileKey, File[] file, Map<String, String> header,
        ...
        try {
            ...
            for (int i = 0; i < fileKey.length; i++) {
                request.part(fileKey[i], file[i].getName(), file[i]);
            }
